...
exports.loginDriver = async (req, res) => {
  try {
    ...
    // Generate JWT token
    const token = jwt.sign({ id: driver._id, email: driver.email }, secretKey, { expiresIn: '1h' });
...
exports.updateDriverLocation = async (req, res) => {
  try {
    ...
    const driver = await Driver.findByIdAndUpdate(driverId, { latitude, longitude }, { new: true });
    if (!driver) {
      return res.status(404).json({ error: 'Driver not found' });
    }
...
exports.updateDriverCarInfo = async (req, res) => {
  try {
    ...
    const driver = await Driver.findByIdAndUpdate(driverId, { carModel, licensePlate }, { new: true });
    if (!driver) {
      return res.status(404).json({ error: 'Driver not found' });
    }
    res.status(200).json(driver);
